{ config, pkgs, ... }: {
  ...
  programs.git = {
    ...
    aliases = {
      ...
      at =
        "for-each-ref --sort=taggerdate --format '%(align:20,left)%(taggerdate:format:%Y-%m-%d  %H:%M)%(end)%(align:18,left)%(tag)%(end)%(align:25,left)%(taggername)%(end)%(subject)' refs/tags";
    };

    userName = "DewaldV";
    userEmail = "dewald.viljoen@pm.me";
    signing = {
      key =
